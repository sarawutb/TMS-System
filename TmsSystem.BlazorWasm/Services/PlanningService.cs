...
{
    public Task<OperationResult<PlanningWorkbenchDto>> GetWorkbenchAsync(CancellationToken cancellationToken = default)
        => GetAsync<PlanningWorkbenchDto>("api/planning/workbench", "Failed to retrieve planning workbench.", cancellationToken);

    public Task<OperationResult<RoutePlanSummaryDto>> GetRoutePlanAsync(long routePlanId, CancellationToken cancellationToken = default)
        => GetAsync<RoutePlanSummaryDto>($"api/planning/routes/{routePlanId}", "Failed to retrieve route plan.", cancellationToken);

    public Task<OperationResult<LoadPlanSummaryDto>> GetLoadPlanAsync(long loadPlanId, CancellationToken cancellationToken = default)
        => GetAsync<LoadPlanSummaryDto>($"api/planning/loads/{loadPlanId}", "Failed to retrieve load plan.", cancellationToken);
    ...
    public Task<OperationResult<RoutePlanSummaryDto>> UpdateRouteStopsAsync(long routePlanId, IReadOnlyList<RouteStopDto> stops, CancellationToken cancellationToken = default)
        => PutAsync<IReadOnlyList<RouteStopDto>, RoutePlanSummaryDto>($"api/planning/routes/{routePlanId}/stops", stops, "Failed to update route stops.", cancellationToken);
